...
class DecagonModel(Model):
    def __init__(self, placeholders, num_feat, nonzero_feat, edge_types, decoders, **kwargs):
        ...
        # self.num_obj_types == 1+1 = 2 ;{(0, 0): 2, (0, 1): 1, (1, 0): 1, (1, 1): 6}
        self.decoders = decoders
        self.inputs = {i: placeholders['feat_%d' % i] for i, _ in self.edge_types}
        # self.input.keys(): dict_keys([0, 1])
    ...
    # build the compute graph for our decagon
    def _build(self):
        self.hidden1 = defaultdict(list)  # make the item of dict default list
        for i, j in self.edge_types:  # for each relation type
            # self.edge_types: {(0, 0): 2, (0, 1): 1, (1, 0): 1, (1, 1): 6}
            # self.edge_types[i,j] :2,1,1,6
            self.hidden1[i].append(GraphConvolutionSparseMulti(  # for each member inside the relation type
        ...
        for i, hid1 in self.hidden1.items():
            self.hidden1[i] = tf.nn.relu(tf.add_n(hid1))  # tf.add_n : add n items in a list

        self.embeddings_reltyp = defaultdict(list)
        for i, j in self.edge_types:
            self.embeddings_reltyp[i].append(GraphConvolutionMulti(  # here i specific a type of node:e.g. gene or drug
        ...
        for i, embeds in self.embeddings_reltyp.items():
            # @rancho: i is key of dic(embeddings_reltyp) and embeds are a list of embedding
        ...
        for edge_type in self.edge_types:
            ...
            # decoder: InnerProductDecoder,DistMultDecoder...
            for k in range(self.edge_types[edge_type]):
                # for each relation in (0,0)/(0,1)/(1,0)/(1,1)
                if decoder == 'innerproduct':
                    ...
                    # Mr in this paper (32,32)
                    glb = self.edge_type2decoder[edge_type].vars['relation_%d' % k]
                    # bilinear don't need loc matrix,so set it to identical matrix (32,32)
                    loc = tf.eye(FLAGS.hidden2, FLAGS.hidden2)
                elif decoder == 'dedicom':
                    # R matrix , globally shared (32,32)
                    glb = self.edge_type2decoder[edge_type].vars['global_interaction']
                    # Dr matrix 1*32->32*32(diagonal)
                    loc = tf.diag(self.edge_type2decoder[edge_type].vars['local_variation_%d' % k])
                else:
                    raise ValueError('Unknown decoder type')
                # At last: latent_interes:(number_relation_types(10),hidden2,hidden2)
                self.latent_inters.append(glb)
                # shape: (10, 32, 32)  = (6+2+1+1,hidden2,hidden2)
