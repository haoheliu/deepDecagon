    ...
    def __init__(self, adj_mats, feat, edge_types, batch_size=100, val_test_size=0.01):
        ...
        for i, j in self.edge_types:
            ...
            for k in range(self.edge_types[i,j]):
                #k relations in a certain kind of vertex
...
#        num_test = max(50, int(np.floor(edges_all.shape[0] * self.val_test_size))) #val_test_size = 0.01
#        num_val = max(50, int(np.floor(edges_all.shape[0] * self.val_test_size)))
        num_test = max(0, int(np.floor(edges_all.shape[0] * self.val_test_size))) #val_test_size = 0.01
        num_val = max(0, int(np.floor(edges_all.shape[0] * self.val_test_size)))
        all_edge_idx = list(range(edges_all.shape[0]))
        np.random.shuffle(all_edge_idx)

        val_edge_idx = all_edge_idx[:num_val] #first part for validation
        val_edges = edges_all[val_edge_idx]

        test_edge_idx = all_edge_idx[num_val:(num_val + num_test)] #second part for test
        test_edges = edges_all[test_edge_idx]

        train_edges = np.delete(edges_all, np.hstack([test_edge_idx, val_edge_idx]), axis=0) #thrird part(remain) for training
#        if(len(train_edges) == 0):
#            print('ERROR:')
#            print(len(edges_all))
#            print(test_edge_idx,val_edge_idx)
            
        test_edges_false = []
        while len(test_edges_false) < len(test_edges):
            idx_i = np.random.randint(0, self.adj_mats[edge_type][type_idx].shape[0])#changed 50 -> 0
...
#        if(len(self.train_edges[edge_type][type_idx]) == 0):
#            print(edge_type,type_idx,'shape:',len(self.train_edges[edge_type][type_idx]))
...
#            print(i,j,k)
#            print(self.batch_num[self.current_edge_type_idx],self.batch_size,len(self.train_edges[i,j][k]),self.batch_size)
